...
feature_num = 80
# 模型结构
pb_path = f'./resources/asr_model_thch30s/mrcldnn'
#pb_path = f'./resources/asr_model_thchs30_cafe/mrdcnn'
...
class SpeechDataset(Dataset):
    def __init__(self, train_data):
        ...
        self.audio_downsample_maxlen = get_audio_len(self.audio_max_len,'mrcldnn')  # 下采样最大长度
    ...
    def __getitem__(self, index):
        ...
        wav_feature_len = np.array([get_audio_len(len(wav_feature),'mrcldnn')])
...
def pre_data():
    train_data = []
    wav_paths = sorted(glob('F:/YYSB/code/code/data1/asr/data_thchs30/train/*.wav'))
    txt_paths = sorted(glob('F:/YYSB/code/code/data1/asr/data_thchs30/train/*.trn'))
    for i in range(len(wav_paths)):
        train_data.append([wav_paths[i], txt_paths[i]])

    test_data = []
    wav_paths = sorted(glob('F:/YYSB/code/code/data1/asr/data_thchs30/test/*.wav'))#('F:/YYSB/code/code/data1/asr/data_thchs30/test-noise/0db/white/*.wav')
    txt_paths = sorted(glob('F:/YYSB/code/code/data1/asr/data_thchs30/test/*.trn'))#data_thchs30/test/*.wav
    for i in range(len(wav_paths)):
...
if __name__ == '__main__':
    ...
    for epoch_index in range(epoch):
        ...
        label_txt, pre_txt = '', ''
        for data in tqdm(
                DataLoader(dataset=SpeechDataset(test_data), batch_size=batch_size, shuffle=True, drop_last=False,
                           num_workers=4, collate_fn=None)):
            wav_features = np.array(data['wav_feature'])
            wav_feature_lens = np.array(data['wav_feature_len'])
            label_ids = np.array(data['label_id'])
