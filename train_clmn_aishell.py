...
batch_size = 32#改
epoch = 40
initial_learning_rate = 6e-4#初始学习率
decay_steps = 5000#衰减速度，训练迭代轮数增加5000，每完整过完一遍训练数据，学习率减少0.95
decay_rate = 0.95#衰减率，
warmup_step = 100000#热身步数
save_step = 200#每200次保存一次
feature_num = 80
# 模型结构
pb_path = f'./resources/asr_model_aishell/mrcldnn'
...
# 构建模型
class AsrModel:
    def __init__(self, voc_num, feature_num,name):
        ...
        asr_output = None
        if name == 'cl':
            asr_output = self.cl_model()

        if name == 'cldnn':
            asr_output = self.cldnn_model()

        if name == 'mrdcnn':
            asr_output = self.mrdcnn_model()

        if name == 'mrcldnn':
            asr_output = self.mrcldnn_model()
    ...
    def cl_model(self):
        ...
        conv_result1 = layers.Conv2D(32, kernel_size=[3, 3], padding='same')(input_features) # 卷积
        conv_result1 = layers.BatchNormalization(epsilon=1e-5)(conv_result1) # bn
        conv_result1 = layers.ReLU()(conv_result1) # 激活函数
        conv_result1 = layers.MaxPool2D((2, 2))(conv_result1) # pool层

        conv_result1 = layers.Conv2D(64, kernel_size=[3, 3], padding='same')(conv_result1) # 同上
        ...
        layer_output = layers.Reshape([-1, conv_result1.shape[2] * conv_result1.shape[3]])(conv_result1) # 同上
    ...
    def cldnn_model(self): # 同上
        input_features = self.audio_mels[:, :, :, tf.newaxis]
        conv_result1 = layers.Conv2D(32, kernel_size=[3, 3], padding='same')(input_features)
        conv_result1 = layers.BatchNormalization(epsilon=1e-5)(conv_result1)
        conv_result1 = layers.ReLU()(conv_result1)
        conv_result1 = layers.MaxPool2D((2, 2))(conv_result1)
        ...
        layer_output = layers.Reshape([-1, conv_result1.shape[2] * conv_result1.shape[3]])(conv_result1)

        layer_output = layers.Bidirectional(layers.LSTM(256, return_sequences=True))(layer_output)
        layer_output = layers.Dense(2048)(layer_output)
        layer_output = layers.Dense(self.voc_num)(layer_output)
...
class SpeechDataset(Dataset): # 加载数据
    def __init__(self, train_data,name):
        super().__init__()
        self.train_data = train_data
        self.name = name
        self.audio_max_len = 640 # 输入最大长度
        self.audio_downsample_maxlen = get_audio_len(self.audio_max_len,name)  # 下采样最大长度
        self.label_max_length = 32 # 输出最大长度
    ...
    def __getitem__(self, index):
        ...
        label_id = self.pad_zero(label_id, self.label_max_length) # 补零

        wav_feature = self.extract_fbank(wav_path)
        wav_feature = wav_feature[:self.audio_max_len]
        wav_feature_len = np.array([get_audio_len(len(wav_feature),self.name)])
    ...
    def pad_zero(self, input, length): # 补零
...
def get_audio_len(audio_len, name): # 计算下采样长度
    audio_len = (audio_len - 2) // 2 + 1
    audio_len = (audio_len - 2) // 2 + 1
    if name == 'mrcldnn' or name == 'mrdcnn':
        audio_len = (audio_len - 2) // 2 + 1
    return audio_len
...
def pre_data():
    train_paths = glob('F:/YYSB/code/code/data1/asr/data_aishell1/wav/train/*/*.wav')
    test_paths = glob('F:/YYSB/code/code/data1/asr/data_aishell1/wav/test/*/*.wav')
    train_dict = {}
    test_dict = {}
    ...
    with open('F:/YYSB/code/code/data1/asr/data_aishell1/transcript/aishell_transcript_v0.8.txt', 'r', encoding='utf-8') as fr:#F:\YYSB\code\code\data1\asr\data_aishell1\transcript
...
if __name__ == '__main__':
    ...
    name = 'mrcldnn'
    asr = AsrModel(char_num, feature_num,name)
    # 恢复权重
    ...
    for epoch_index in range(epoch):
        loader = DataLoader(dataset=SpeechDataset(train_data,name), batch_size=batch_size, shuffle=True, drop_last=False,
        ...
        label_txt, pre_txt = '', ''
        for data in tqdm(
                DataLoader(dataset=SpeechDataset(test_data,name), batch_size=batch_size, shuffle=True, drop_last=False,
                           num_workers=4, collate_fn=None)):
            wav_features = np.array(data['wav_feature'])
            wav_feature_lens = np.array(data['wav_feature_len'])
            label_ids = np.array(data['label_id'])
            label_lens = np.array(data['label_len'])
            asr_output = asr_train_model(wav_features, training=False)
