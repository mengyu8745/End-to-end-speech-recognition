...
feature_num = 80
# 模型结构
pb_path = f'./resources/asr_model2/cldnn'
...
class AsrModel:
    def __init__(self, voc_num, feature_num,name):
        ...
        self.audio_mels = layers.Input(shape=[1200, feature_num], name='audio_mels')
...
class SpeechDataset(Dataset):
    def __init__(self, train_data):
        ...
        self.audio_downsample_maxlen = get_audio_len(self.audio_max_len,'cldnn')  # 下采样最大长度#'cl'
    ...
    def __getitem__(self, index):
        ...
        wav_feature_len = np.array([get_audio_len(len(wav_feature),'cldnn')])
...
def pre_data():
    ...
    test_data = []
    wav_paths = sorted(glob('F:/YYSB/code/code/data1/asr/data_thchs30/test/*.wav'))
    txt_paths = sorted(glob('F:/YYSB/code/code/data1/asr/data_thchs30/test/*.trn'))
    for i in range(len(wav_paths)):
